 ...
 * Model Menu
 */
class M_ticket_machine extends CI_Model{

    public function get_data()
    {
        $this->db->select('ticket_machine.*, users.employee_no, users.first_name, assigned_opt.name as name_dept, groups.description');
        $this->db->from('ticket_machine');
        $this->db->join('users','users.id = ticket_machine.id_request',"left");
    ...
    {
        return $this->db->get_where('ticket_machine', array('ticket_no' => $ticket_no))->num_rows();
    }

    public function insert_ticket($data)
	{
		return $this->db->insert('ticket_machine', $data);
	}
    ...
    public function update_status($data, $ticket_no)
    {
		...
		return $this->db->update('ticket_machine', $data);
    }

    public function delete_tt_machine($id)
    {
        $this->db->where('ticket_no', $id);
		$this->db->delete('ticket_machine');
    }
