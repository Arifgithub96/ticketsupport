...
class Ion_auth_model extends CI_Model
{
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description,' .$this->tables['groups'].'.id_assigned_opt,' .$this->tables['groups'].'.function,' .$this->tables['groups'].'.group')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
		                ->get($this->tables['users_groups']);
	}

	public function get_name_user($id = FALSE)
	{
		// $this->trigger_events('get_users_group');
	...
	public function get_image_profil($id = FALSE)
	{
		// $this->trigger_events('get_users_group');
		...
		// return $this->db->select('u_user_register.file_picture')
		//                 ->where('users.id',$id)
		//                 ->join('u_user_register','u_user_register.id = users.id_uregister','left')
		//                 ->get('users');

		return $this->db->select('employees.file_picture')
		                ->where('users.id',$id)
		                ->join('employees','employees.employee_no = users.employee_no','left')
		                ->get('users');
