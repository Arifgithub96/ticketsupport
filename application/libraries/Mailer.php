...
class Mailer {
    protected $_ci;
    protected $email_pengirim = 'troubleticket@asiagalaxy.com'; // Isikan dengan email pengirim
    protected $nama_pengirim = 'Info Trouble Ticket'; // Isikan dengan nama pengirim
    protected $password = 'abcde123!@#'; // Isikan dengan password email pengirim
    ...
    public function send($data, $mailto='', $ccmail=''){
        $mail = new PHPMailer;
        $mail->isSMTP();

        $mail->Host = 'srv98.niagahoster.com';
        ...
        foreach ($mailto as $value) {//email penerima
            $mail->addAddress($value, '');
        }

        foreach ($ccmail as $value) {//email cc
            $mail->addCC($value, '');
        }
        $mail->addAddress($data['email_penerima'], '');
        // $mail->addCC($data['cc_satu'], '');
        $mail->isHTML(true); // Aktifkan jika isi emailnya berupa html

        $mail->Subject  = $data['subjek'];
        $mail->Body     = $data['content'];
        $mail->AddEmbeddedImage('image/icon.jpg', 'Galaxy Investasi Harapan', 'icon.jpg'); // Aktifkan jika ingin menampilkan gambar dalam email
