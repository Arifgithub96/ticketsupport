...
class Machine extends CI_Controller {
	...
	public function __construct(){
	    ...
	    $this->load->model('m_ticket_machine');
	...
	{
        $this->data['get_data']     = $this->m_ticket_machine->get_data();
		$this->admintemp->view('backend/machine/machine', $this->data);
	}
    ...
    public function create()
    {
        if($this->input->post()){
            ...
            do 
            {
                ...
                $chek_ticket = $this->m_ticket_machine->check_noticket($ticket_no);
                // log_r($chek_ticket."  ".$i);
                $i++;
            } while($chek_ticket > 0);
            ...
            // log_r($data);
            $status = $this->m_ticket_machine->insert_ticket($data);
            // $status =1;
            if($status == 1)
            { 
            ...
            $this->data['get_user'] 		= $this->m_ticket_machine->get_user();
            // log_r($this->data['get_user']);
            $this->admintemp->view('backend/machine/create', $this->data);
        }
    }
    ...
    function mail_create($data='')//send email new ticket
    {
	    ...
	    $get_req    = $this->m_ticket_machine->get_user_row($id_request);//get requestor id
		$email_req  = $get_req->email;
		$this->data['name_req']  =  $get_req->first_name;
		$this->data['desc_req']  =  $get_req->description;

		$get_to   = $this->m_ticket_machine->get_user_row(2);//get to id
		...
		$content = $this->load->view('backend/machine/mail_create', $this->data, true);//view email
		$mailto = array(
    ...
    public function acceptance($id='')
    {
        if($this->input->post()){
            ...
            // log_r($data);
            $this->m_ticket_machine->update_status($data, $ticket_no);
            ...
            $this->data['get_row_ticket'] = $this->m_ticket_machine->m_row_ticket($id);//id = ticket_no
            // log_r($this->data['get_row_ticket']);
            $this->admintemp->view('backend/machine/accept', $this->data);
        }
    }
    ...
    {   
        ...
        $data_ticket = $this->m_ticket_machine->m_row_ticket($ticket_no);//id = ticket_no
		...
		$get_admin      = $this->m_ticket_machine->get_user_row(2);//Action From Admin
		...
		$content = $this->load->view('backend/machine/mail_discussion', $this->data, true);//view email
		$mailto = array(
			'penerima_satu' => $email_req,//email_req
            // 'penerima_satu' => $email_admin,//email_req
		);
    ...
    function send_closed($data='', $ticket_no='')
    {
		...
		$content = $this->load->view('backend/machine/mail_closed', $this->data, true);//view email
		$mailto = array(	
			'penerima_satu' => $email_req,//email_req
            // 'penerima_satu' => $email_admin,//email_req
		);
    ...
    {
        $this->data['get_user'] 	  = $this->m_ticket_machine->get_user();
        $this->data['get_row_ticket'] = $this->m_ticket_machine->m_row_ticket($ticket_no);//id = ticket_no
        // log_r($this->data['get_row_ticket']);
        $this->data['date_discuss'] = strftime('%Y-%m-%dT%H:%M:%S', strtotime($this->data['get_row_ticket']->date_discuss));
        $this->data['date_progress'] = strftime('%Y-%m-%dT%H:%M:%S', strtotime($this->data['get_row_ticket']->date_progress));
        $this->admintemp->view('backend/machine/detail_ticket', $this->data);
    }

    public function delete($id)
    {
        $this->m_ticket_machine->delete_tt_machine($id);
    }
